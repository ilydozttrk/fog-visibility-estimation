"""
evaluation_resnet50.py

Evaluation pipeline for the pretrained ResNet50 baseline model
...
from src.training.train_resnet50 import (
    build_resnet50_regression_model,
)
...
def load_trained_model(
    checkpoint_path: Path = RESNET50_CHECKPOINT_PATH,
) -> tuple[nn.Module, dict]:
    """
    Load the trained ResNet50 regression model from a checkpoint.
    ...
    model = build_resnet50_regression_model()
...
def save_evaluation_summary(
    ...
    validation_mae = checkpoint.get("validation_mae")

    if validation_mae is None:
        raise KeyError(
            "Checkpoint does not contain 'validation_mae'."
        )
...
def plot_actual_vs_predicted(
    ...
    plt.title("ResNet50 Predictions vs Ground Truth")
...
def plot_prediction_error_histogram(
    ...
    plt.title("ResNet50 Prediction Error Distribution")
...
def save_evaluation_report(
    ...
    """
    Save a human-readable Markdown report for the
    ResNet50 evaluation.
    """
    ...
    lines = [
        "# ResNet50 Baseline Evaluation Report",
        "",
        "## Model Information",
        "",
        "- Model: ResNet50",
        "- Pretrained weights: ImageNet",
        f"- Checkpoint path: `{RESNET50_CHECKPOINT_PATH}`",
        ...
        target_comment,
        "",
        "This result represents the initial ResNet50 baseline "
        "under the same dataset split, preprocessing pipeline, "
        "training duration, and evaluation settings used for VGG16.",
        "",
        "## Generated Outputs",
        "",
        "- `results/evaluation/resnet50_test_predictions.csv`",
        "- `results/evaluation/resnet50_evaluation_summary.json`",
        "- `results/plots/resnet50_actual_vs_predicted.png`",
        "- `results/plots/resnet50_prediction_error_histogram.png`",
        "",
        "## Next Stage",
        "",
        "The next stage is to compare the VGG16 and ResNet50 baseline "
        "models using validation MAE, test MAE, signed-error behaviour, "
        "absolute-error distributions, and prediction plots. The model "
        "with the lower independent test MAE will be selected as the "
        "baseline candidate for the attention mechanism stage.",
    ]
...
def evaluate_resnet50() -> None:
    """
    Run the complete ResNet50 test evaluation pipeline.
    """
    ...
    print("=" * 70)
    print(
        "ResNet50 Fog Visibility Regression Evaluation"
    )
    print("=" * 70)
    ...
    checkpoint_epoch = checkpoint.get("epoch")
    checkpoint_validation_mae = checkpoint.get(
        "validation_mae"
    )

    if checkpoint_validation_mae is None:
        raise KeyError(
            "Checkpoint does not contain 'validation_mae'."
        )
    ...
    print(
        ...
        f"Validation MAE: "
        f"{float(checkpoint_validation_mae):.4f} m)"
    )
