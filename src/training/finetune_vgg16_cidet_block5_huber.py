"""
finetune_vgg16_cidet_block5_huber.py

Controlled CIDET fine-tuning experiment using Huber/Smooth L1 loss.
...
- train Block 5 with learning rate 1e-5
- train the regression head with learning rate 1e-4
- use normal CIDET training sampling
- optimize SmoothL1Loss with beta=200 metres
- report true MAE independently from the optimization loss
- select the best checkpoint using true CIDET validation MAE
...
CIDET_BLOCK5_HUBER_CHECKPOINT_PATH = (
    PROJECT_ROOT
    / "results"
    / "checkpoints"
...
def verify_trainable_parameters(
    ...
    if any(
        ...
        raise RuntimeError(
            "A parameter in VGG16 Blocks 1-4 "
            "is unexpectedly trainable."
        )
...
def train_one_epoch(
    ...
    Returns:
        mean optimization loss,
        true mean absolute error in metres.
    """
    model.train()
    ...
    mean_mae = (
        running_absolute_error / total_samples
    )
...
def save_checkpoint(
    ...
    """Save the best validation-MAE-selected Huber checkpoint."""
    CIDET_BLOCK5_HUBER_CHECKPOINT_PATH.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    checkpoint = {
        ...
        "huber_beta_m": HUBER_BETA,
        "sampling_strategy": "standard training sampling",
...
def main() -> None:
    ...
    print(
        f"Output checkpoint:   "
        f"{CIDET_BLOCK5_HUBER_CHECKPOINT_PATH}"
    )
    print("Blocks 1-4 frozen:   True")
    print("Block 5 trainable:   True")
    print("Sampling:            Standard")
    ...
    for epoch in range(
        ...
        print(
            ...
            f"| Train Huber: {training_loss:.4f} "
            f"| Train MAE: {training_mae:.4f} m "
            f"| Val Huber: {validation_loss:.4f} "
            f"| Val MAE: {validation_mae:.4f} m"
        )
    ...
    if best_validation_mae < 326.6640:
        print(
            "Experiment 4 improved over "
            "the Block 5 reference."
        )
    else:
        print(
            "Experiment 4 did not improve over "
            "the Block 5 reference."
        )

    print(
        "\nCIDET test split remains untouched."
    )
