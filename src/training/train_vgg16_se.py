...
Training pipeline for the SE-enhanced VGG16 regression model
used in fog visibility estimation.

The training configuration is intentionally kept consistent with
the VGG16 baseline and VGG16 + CBAM experiments so that the effect
of the attention mechanism can be evaluated under comparable
...
def train_one_epoch(
    ...
    """
    Train the SE-enhanced model for one epoch
...
def save_checkpoint(
    ...
    """
    Save the best SE model checkpoint
...
def record_experiment(
    ...
    """
    Append the completed SE experiment
    ...
    worksheet.append(
        [
            ...
            (
                "VGG16 baseline enhanced with SE channel attention. "
                "SE inserted before the final VGG16 MaxPool as the "
                "proposal risk-management alternative to CBAM."
            ),
        ]
    )
...
def save_initial_training_log(
    ...
    initial VGG16 + SE training run.
    """

    lines = [
        "# Initial VGG16 + SE Training Log",
        "",
        ...
        (
            "- Experimental role: "
            "risk-management alternative defined in the "
            "accepted proposal after CBAM failed to improve "
            "test MAE"
        ),
        "",
        "## Epoch Results",
    ...
    lines.extend(
        [
            ...
            (
                "- Checkpoint: "
                f"`{VGG16_SE_CHECKPOINT_PATH}`"
            ),
            "",
            "## Initial Observation",
            "",
            (
                "The VGG16 + SE training pipeline "
                "completed successfully. "
                "The checkpoint corresponding to the "
                "lowest validation MAE was saved for "
                "subsequent independent test evaluation "
                "and comparison with the VGG16 baseline "
                "and VGG16 + CBAM experiments."
            ),
        ]
    )
...
def train_model() -> None:
    """
    Run the complete VGG16 + SE training pipeline.
    """
    ...
    initial_training_log_path = (
        logs_dir
        / "vgg16_se_initial_training_log.md"
    )

    experiment_id = datetime.now().strftime(
        "VGG16_SE_%Y%m%d_%H%M%S"
    )
    ...
    print("=" * 70)
    print(
        "VGG16 + SE Fog Visibility Regression Training"
    )
    print("=" * 70)
